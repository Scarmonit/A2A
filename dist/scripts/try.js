...
async function main() {
    ...
    if (!res.ok) {
        ...
        throw new Error(`Demo invoke failed: ${res.status} ${t}`);
    }
    const body = await res.json();
    if (!body.ok)
        throw new Error(`Invoke error: ${body.error?.message || 'unknown'}`);
    const data = body.data;
    const streamUrl = data.streamUrl;
    console.log('[try] streamUrl =', streamUrl);
    // Attach token if not already present
    const url = new URL(streamUrl);
    if (!url.searchParams.get('token') && STREAM_TOKEN)
    ...
    await new Promise((resolve, reject) => {
        ...
        ws.on('message', (buf) => {
            ...
            if (ev.type === 'final') {
                console.log('\n[try] final:', JSON.stringify(ev.result));
                resolve();
                ws.close();
            }
            if (ev.type === 'error') {
                console.error('\n[try] error:', ev.message);
