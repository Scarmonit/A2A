...
export class AgentRegistry {
    agents = new Map();
    tagIndex = new Map(); // tag -> agent IDs
    categoryIndex = new Map(); // category -> agent IDs
    constructor() {
    ...
    list(filter) {
        ...
        if (filter?.tags && filter.tags.length > 0) {
            candidates = candidates.filter(a => filter.tags.some(tag => a.tags?.includes(tag)));
        }
        if (filter?.search) {
            const search = filter.search.toLowerCase();
            candidates = candidates.filter(a => a.name.toLowerCase().includes(search) ||
    ...
    generateCapabilitiesForType(agentType) {
        switch (agentType) {
            case 'file-ops':
                return [
                    {
