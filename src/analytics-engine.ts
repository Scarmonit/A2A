...
export class AnalyticsEngine {
  ...
  private sampleRate = Number(process.env.ANALYTICS_SAMPLE_RATE || 1)
  private maxQueryScan = Number(process.env.ANALYTICS_MAX_QUERY_SCAN || 50000)
  ...
  track(event: Omit<AnalyticsEvent, 'timestamp'>): void {
    try {
      ...
      logger.warn({ err }, 'Failed to track analytics event')
    }
  }
