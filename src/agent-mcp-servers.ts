...
export type MCPServerConfig = {
  maxClients: number;
  allowedAgents?: string[]; // If specified, only these agents can connect
  requireAuth: boolean;
  rateLimits: {
...
export class AgentMCPServerManager extends EventEmitter {
  private servers = new Map<string, AgentMCPServer>();
  private portAllocator = new Set<number>();
  private nextPort = 8800; // Start from 8800 for agent servers
  private sharingAgreements = new Map<string, ToolSharingAgreement>();
  private toolRegistry = new Map<string, { agentId: string; tool: AgentTool }>();
  ...
  async createAgentMCPServer(
    ...
    const agentServer: AgentMCPServer = {
      agentId,
      port,
      tools: new Map(),
      clients: new Set(),
  ...
  async shareToolWithAgent(
    ...
    const agreement: ToolSharingAgreement = {
      id: agreementId,
      providerAgentId,
      consumerAgentId,
      toolName,
  ...
  async executeSharedTool(
    ...
    const agreement = Array.from(this.sharingAgreements.values()).find(a =>
      a.providerAgentId === providerAgentId &&
  ...
  discoverTools(filters: {
    ...
    for (const [key, entry] of this.toolRegistry.entries()) {
      ...
      // Apply filters
      if (filters.agentType && !agentId.includes(filters.agentType)) continue;
      if (filters.maxCost !== undefined && (tool.cost || 0) > filters.maxCost) continue;
      
      // Check if any sharing agreement exists or tool is freely shareable
      const hasSharing = Array.from(this.sharingAgreements.values()).some(a =>
        a.providerAgentId === agentId && a.toolName === tool.name && a.active
      );
  ...
  getAgentMCPServer(agentId: string): AgentMCPServer | undefined {
    return this.servers.get(agentId);
  }

  // Get all sharing agreements for an agent
  getSharingAgreements(agentId: string): {
    providing: ToolSharingAgreement[];
    consuming: ToolSharingAgreement[];
  } {
    const providing = [];
    const consuming = [];
  ...
  async stopAgentMCPServer(agentId: string): Promise<{ success: boolean; error?: string }> {
    const server = this.servers.get(agentId);
    if (!server) {
  ...
  private async spawnAgentMCPProcess(
    agentId: string, 
    port: number, 
    config: MCPServerConfig
...
`;
  ...
  private async createMCPConnection(clientAgentId: string, port: number): Promise<any> {
    return new Promise((resolve, reject) => {
      const client = net.createConnection({ port }, () => {
        resolve({
          send: (data: any) => client.write(JSON.stringify(data)),
          close: () => client.end()
        });
      });
  ...
  private cleanupExpiredAgreements(): void {
    const now = Date.now();
