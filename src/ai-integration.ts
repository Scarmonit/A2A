...
export type AICapability = {
  ...
  systemPrompt?: string;
  config: LLMConfig;
  tools?: string[]; // Tool names the AI can use
...
export class AIIntegration {
  ...
  async generateResponse(
    ...
    input: any, 
    context?: { agentId: string; requestId: string; sessionHistory?: any[] }
  ): Promise<{ 
    response: string; 
    toolsUsed: string[]; 
  ...
  private buildMessages(capability: AICapability, input: any, sessionHistory?: any[]): any[] {
    const messages = [];
  ...
  private formatPrompt(template: string, input: any): string {
  ...
  private async callOpenAI(params: any): Promise<{ response: string; tokens: any; toolsUsed: string[] }> {
  ...
  private async callAnthropic(params: any): Promise<{ response: string; tokens: any }> {
  ...
  private async callLocalLLM(params: any): Promise<string> {
    ...
    try {
      const response = await fetch(`${baseUrl}/api/generate`, {
        ...
        body: JSON.stringify({
          model: params.model,
          prompt: params.messages.map((m: any) => `${m.role}: ${m.content}`).join('\n'),
          options: {
            temperature: params.temperature,
  ...
  private buildOpenAIFunctions(toolNames: string[]): any[] {
