...
// SSE Transport for Claude.ai connectors
const SSE_PORT = parseInt(process.env.SSE_PORT || '8788', 10);
const SSE_ENABLED = process.env.SSE_ENABLED !== 'false';

if (SSE_ENABLED) {
  const sseTransport = new SSEServerTransport(server, SSE_PORT);
  sseTransport.start();
  logger.info({ port: SSE_PORT }, 'SSE transport enabled for Claude.ai');
}
